...
- Keep response DTOs purpose-specific; use separate list and detail shapes when the fields differ.
- Keep collection responses paginated or explicitly bounded, with ordering documented in the contract.
- Do not expose Prisma models, ORM records, persistence records, or arbitrary `select`/`include` controls through API responses.
